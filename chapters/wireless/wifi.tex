...
\begin{section}{WLAN security}
  ...
  \begin{subsection}{WPA}
    WEP is used only im the 802.11 vanilla, but is was soon replaced by 802.11 with the WPA(Wi-Fi
    ...
    \begin{subsubsection}{802.1X authentication model}
      ...
      \end{itemize}
      A supplicant to authenticate itself to the network, has to authenticate itself to the authenticator 
      at first. If the authentication is successful, the authenticator will instructs the authenticator 
      to switch the port on. The authentication server will then inform the supplicant that the 
    ...
    \begin{subsubsection}{Key management}
      A quick reminder that in WEP, all the STA share the same key. In WPA, this is not the case:
      to derive multiple keys from the master key, a \textbf{Pairwise Master Key(PMK)} is used, which
    ...
    \begin{subsubsection}{Four-way handshake}
      ...
      At first, the AP sends a nonce to the client in an unencrypted message and without any integrity
      check.\\
      The client receives message and derives the $PTK=f(\text{PMK},\text{AP}_{\text{nonce}},\text{STA}_{\text{nonce}}
      ...
      check, which is computed using half of the $PTK$ to protect the message and avoid brute force
      attacks on the $PTK$. This message too is unencrypted.\\
      Once this message is received, the AP computes the $PTK$ after having verified the integrity of the
      message, and sends the third message, which contains the GTK and its associated MIC(message integrity
      code), which is computed using the other half of the $PTK$. This message acts also as a request 
      to install the key. After the client receives this message, it the MIC checks out, it means that
      the AP and the client have the same $PTK$, and the GTK is installed.\\
      One last message is sent by the client, which is an acknowledgment of the GTK installation, with 
      its associated MIC. If the integrity check is successful, the AP installs the GTK and the PTK, and
      the client is authenticated, concluding the four-way handshake.
    ...
    \begin{subsubsection}{Bruteforce attack against WPA}
      ...
      This kind of attack is possible only if the key is weak, this is because in WPA2 the PMK is 
      derived from the password with PBKDF2, which is a key derivation function. This brings the
      complexity of the attack to $2^{256}$ from $2^{48}$, which is quite impossible to perform.
    \end{subsubsection}
    \begin{subsubsection}{TKIP mode}
      TKIP is a protocol that was introduced to allow WPA to be compatible with WEP, but correct some
      of its flaws:
      \begin{itemize}
    ...
    \begin{subsubsection}{Simultaneous Authentication of Equals}
      ne of the major new features introduced in WPA3 is Simultaneous Authentication of Equals (SAE),
